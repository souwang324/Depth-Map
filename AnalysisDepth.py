...
def main():
    file_name1 = get_program_parameters()

    colors = vtkNamedColors()

    img = cv2.imread(file_name1, cv2.IMREAD_UNCHANGED)
    height, width1, dim = img.shape
    #print(img.shape)
    width = width1>>1
    tex = img[:, 0:width, :]
    #print(src.shape)
    src = img[:, width:width1, 0]
    ...
    for r in range(height):
        for c in range(width):
            points.InsertNextPoint(r, c, src[r, c])
    ...
    renWin.SetWindowName('Depth Map - Point')
